cd ../build && make -j \
  && make install \
  && cd ../tmp \
  && METADATA=note.yaml KO_CC=clang-12 KO_USE_Z3=1 KO_DONT_OPTIMIZE=1 ../build/bin/ko-clang -S -emit-llvm -o linklist-O0.ll -g linklist.c \
  && METADATA=note.yaml KO_CC=clang-12 KO_USE_Z3=1 KO_DONT_OPTIMIZE=1 ../build/bin/ko-clang -g -o binary/linklist linklist.c \
